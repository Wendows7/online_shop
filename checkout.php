...
if (@$_SESSION['cart']) {
   $unik = "KODE-" . random_int(00000, 99999);

   $json = json_encode($_SESSION["cart"]);

   $createdAt = date('Y-m-d H:i:s');

   $sql = mysqli_query($con, "INSERT INTO keranjang (kode, detail_keranjang, created_at)
    VALUES ('$unik', '$json', '$createdAt')");



   unset($_SESSION['cart']);

   $query = mysqli_query($con, "SELECT * FROM keranjang WHERE kode='$unik' ");
   ...
   if (!file_exists($penyimpanan))
      mkdir($penyimpanan);
   $isi = $hasil['kode'];
   $qrkode = QRcode::png($isi, $penyimpanan . "/qr-$unik.png", "H", 6, 4);
} else {
         ...
         <div class="col-sm-12 col-md-12 col-lg-12 text-center">
            <div class="box">
               ...
               <div class="img-box " style="margin: 50px; padding:10px">
                  <img src="temp/qr-<?= $unik ?>.png" width="300" height="300" class="rounded border border-primary">
                  <div class="detail-box"><b><?= $unik ?></b>
                     <br>
                     <br><b>Screenshot Dan Tunjukan QRCODE Ini Ke Kasir Untuk Melakukan Pembayaran
                        </br></b>
                  </div>
               </div>
            </div>
