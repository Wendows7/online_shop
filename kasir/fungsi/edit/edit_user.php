...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['pengaturan'])){
		...
		$row -> execute($data);
		echo '<script>window.location="../../index_user.php?page=pengaturan&success=edit-data"</script>';
	}
	...
	if(!empty($_GET['kategori'])){
		...
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index_user.php?page=kategori&uid='.$id.'&success-edit=edit-data"</script>';
	}
	...
	if(!empty($_GET['stok'])){
		...
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index_user.php?page=barang&success-stok=stok-data"</script>';
	}
	...
	if(!empty($_GET['barang'])){
		...
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index_user.php?page=barang/edit&barang='.$id.'&success=edit-data"</script>';
	}
	...
	if(!empty($_GET['gambar'])){
		...
		if ($_FILES['foto']["error"] > 0) {
			...
			echo '<script>alert("You can only upload JPG, PNG and GIF file");window.location="../../index_user.php?page=user"</script>';
		}elseif (round($_FILES['foto']["size"] / 1024) > 4096) {
			// echo "WARNING !!! Besar Gambar Tidak Boleh Lebih Dari 4 MB";
			// echo "<font face='Verdana' size='2' ><BR><BR><BR>
			// 		<a href='../../index.php?page=user'>Back to upform</a><BR>";
			echo '<script>alert("WARNING !!! Besar Gambar Tidak Boleh Lebih Dari 4 MB");window.location="../../index_user.php?page=user"</script>';
		}else{
            ...
            if(move_uploaded_file($tmp_name, $dir.$name)){
				...
				$row = $config -> prepare($sql);
				$row -> execute($data);
				echo '<script>window.location="../../index_user.php?page=user&success=edit-data"</script>';
			}else{
				echo '<script>alert("Masukan Gambar !");window.location="../../index_user.php?page=user"</script>';
			}
		}
	}
	...
	if(!empty($_GET['jual'])){
		...
		if($hasil['stok'] > $jumlah)
		{
			...
			$row1 -> execute($data1);
			echo '<script>window.location="../../index_user.php?page=jual#keranjang"</script>';
		}else{
			echo '<script>alert("Keranjang Melebihi Stok Barang Anda !");
					window.location="../../index_user.php?page=jual#keranjang"</script>';
		}
	...
	?>
		...
		<?php foreach($hasil1 as $hasil){?>
			<tr>
				...
				<td>
				<a href="fungsi/tambah/tambah_user.php?jual=jual&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>" 
